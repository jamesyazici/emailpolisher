...
type GuardrailsConfig = z.infer<typeof GuardrailsSchema>;

let guardrailsCache: GuardrailsConfig | null = null;

function getGuardrails(): GuardrailsConfig {
  if (!guardrailsCache) {
...
function getAllText(draft: DraftOutput): string {
  return [
    draft.subject,
    draft.greeting,
    ...draft.bodySections,
...
function getWordCountBand(wordCount: number): 'short' | 'medium' | 'long' | 'very-long' {
...
function analyzeSentences(text: string): { count: number; avgLength: number; longCount: number } {
...
function countSlangHits(text: string): number {
...
function countEmojiHits(text: string): number {
  const guardrails = getGuardrails();
...
function countOverpromiseHits(text: string): number {
  const guardrails = getGuardrails();
...
function countAttachmentRefs(text: string): number {
  const guardrails = getGuardrails();
  const lowerText = text.toLowerCase();
...
function countLinkRefs(text: string): number {
  const guardrails = getGuardrails();
...
function calculateReadabilityScore(metrics: Partial<EvalMetrics>): number {
  let score = 100;
...
function calculateProfessionalismScore(metrics: Partial<EvalMetrics>): number {
  let score = 100;
...
function calculateOverallScore(readabilityScore: number, professionalismScore: number): number {
  // Weighted average: 60% professionalism, 40% readability
  return Math.round(professionalismScore * 0.6 + readabilityScore * 0.4);
}

export function evalDraft(draft: DraftOutput, checks: CheckResult): EvalMetrics {
  ...
  const metrics: Partial<EvalMetrics> = {
    wordCount,
    wordCountBand,
