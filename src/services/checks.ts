import { DraftOutput, DraftInput, CheckResult } from '../types/domain.js';
...
function checkCompleteness(draft: DraftOutput): string[] {
...
function checkProfessionalism(draft: DraftOutput): string[] {
  const warnings: string[] = [];
...
function checkClarity(draft: DraftOutput): string[] {
  const warnings: string[] = [];
  const allText = getAllText(draft);
...
function checkEthical(draft: DraftOutput): string[] {
  const warnings: string[] = [];
  const guardrails = getGuardrails();
...
function checkAttachmentLinkConsistency(draft: DraftOutput): string[] {
  const warnings: string[] = [];
  const guardrails = getGuardrails();
  const allText = getAllText(draft);
  
  // Check for attachment references
  const hasAttachmentKeywords = guardrails.attachment_keywords.some(keyword => 
...
export function runChecks(draft: DraftOutput, input: DraftInput): CheckResult {
  const warnings: string[] = [];
