...
import { llmClient, LLMCompletionParams } from './llm.js';
...
export interface RefineResult {
  draft: DraftOutput;
  checks: CheckResult;
  evalMetrics: any; // EvalMetrics from evalChecks
  wasRefined: boolean;
 ...
 */
function generateUserPrompt(draft: DraftOutput, tone: ToneSettings, category: EmailCategory): string {
 ...
 */
function parseLLMResponse(response: string): { email: string; evaluation: string } | null {
 ...
 */
function parseEmailText(emailText: string): DraftOutput | null {
 ...
 */
function hasHighSeverityIssues(checks: CheckResult, evalMetrics: any): boolean {
...
export async function refineWithLLM(
  ...
  try {
    ...
    // Call LLM
    const llmParams: LLMCompletionParams = {
      systemPrompt,
      userPrompt,
      temperature: 0.7,
      maxTokens: 1000
