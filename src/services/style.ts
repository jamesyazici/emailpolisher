import { DraftOutput, ToneSettings } from '../types/domain.js';
...
type StyleLexicons = z.infer<typeof StyleLexiconsSchema>;

let styleLexiconsCache: StyleLexicons | null = null;

function getStyleLexicons(): StyleLexicons {
...
function applyFormality(text: string, formalityLevel: number): string {
...
function applyWarmth(bodySections: string[], warmthLevel: number): string[] {
  const lexicons = getStyleLexicons();
...
function applyConfidence(text: string, confidenceLevel: number): string {
  const lexicons = getStyleLexicons();
  let result = text;
...
function applySeniority(bodySections: string[], seniority: 'student' | 'professional'): string[] {
  const lexicons = getStyleLexicons();
  const result = [...bodySections];
...
function applyLength(text: string, length: 'short' | 'medium' | 'long'): string {
  let result = text;
...
export function applyStyle(draft: DraftOutput, tone: ToneSettings): DraftOutput {
